...
# Versão: 6.2.0 - Adicionada Funcionalidade de Salvar/Carregar Sessão
# Autor: jefferson/configexe
#
# Melhorias da Versão 6.2.0:
# - NOVO RECURSO: Adicionado o botão "Salvar Sessão" que permite salvar o estado
#   atual da aplicação (dados, tabelas carregadas, gráficos, formatação)
#   em um único arquivo HTML.
# - NOVO RECURSO: Ao abrir um arquivo HTML salvo, a aplicação é restaurada
#   exatamente ao estado em que foi salva, permitindo a continuidade da análise.
# - A versão 6.1.1 continua com a correção para gráficos do tipo "Combo".
# -----------------------------------------------------------------------------
...
Function Get-HtmlTemplate {
    ...
    // ---------------------------------------------------
    // Power-Graphx Web App - Lógica Principal (v6.2.0)
    // ---------------------------------------------------
    ...
    window.onload = () => {
        try {
            ...
            initializeApp();
            loadStateAndRestoreUI(); // Tenta carregar um estado salvo
        } catch (e) {
    ...
    function initializeApp() {
        // Se houver um estado salvo, os dados virão dele. Se não, usa os dados injetados.
        const jsonDataEl = document.getElementById('jsonData');
        if (jsonDataEl) {
           originalData = JSON.parse(jsonDataEl.textContent);
        } else {
           originalData = []; // Inicia vazio se não houver dados iniciais
        }
    ...
    function updateTableListUI() {
        ...
        document.querySelectorAll('.chart-data-source, #calc-column-table').forEach(select => {
            ...
            if (select.id !== 'calc-column-table' && select.value) {
    ...
    function setupEventListeners() {
        ...
        document.getElementById('btn-save-session').addEventListener('click', saveState);
        ...
        document.getElementById('btn-reset-data').addEventListener('click', () => {
            if (!document.getElementById('jsonData')) {
                alert("Não há dados originais para restaurar. Recarregue o arquivo CSV.");
                return;
            }
    ...
    function handleCellEdit(e) {
        ...
        if(rowIndex >= 0 && colName && currentData[rowIndex]) {
            ...
            // This is tricky. Which table to update? For now, assume it's the view.
            // A more robust solution would track the source table of `currentData`.
            document.querySelectorAll('.chart-analysis-section').forEach(section => renderChart(section.dataset.id));
        }
    }
    ...
    function renderTable() {
        ...
        if (!currentData || currentData.length === 0) {
            tableContainer.innerHTML = `<p class="text-center text-gray-500 p-8">Nenhum dado para exibir. Carregue um CSV ou execute uma consulta SQL.</p>`;
            return;
        }
        ...
        currentData.forEach((row, rowIndex) => {
            ...
            columnStructure.forEach(col => {
                ...
                applyCellFormatting(td, row[col.originalName], col.originalName);
                tr.appendChild(td);
            });
    ...
    function addCondFormatRule() {
        ...
        conditionalFormattingRules.push({ id: Date.now(), column, condition, value, color });
        renderCondFormatRules();
        renderTable();
    }
    ...
    function renderCondFormatRules() {
        ...
        conditionalFormattingRules.forEach(rule => {
            ...
            ruleDiv.innerHTML = `<div class="flex items-center text-sm">
                ...
                <span>Se <b>${rule.column}</b> ${rule.condition.replace('_', ' ')} <b>${rule.value}</b></span>
            </div>
    ...
    function applyCellFormatting(cell, cellValue, columnName) {
    ...
    function addChartAnalysis(id = null) {
        if (id === null) {
            chartAnalysisCounter++;
            id = chartAnalysisCounter;
        } else {
            // Ensure the global counter is ahead of any loaded IDs
            chartAnalysisCounter = Math.max(chartAnalysisCounter, id);
        }
        const template = document.getElementById('chart-analysis-template').innerHTML;
        const newChartHtml = template.replace(/__ID__/g, id);
        const container = document.getElementById('charts-container');
        const div = document.createElement('div');
        div.innerHTML = newChartHtml;
        container.appendChild(div.firstElementChild);
        initializeChartUI(id);
    }
    ...
    function initializeChartUI(id) {
        ...
        updateTableListUI(); // Populates the data source select
    }
    ...
    function buildChartOptions(section, datasets) {
        const chartTitle = section.querySelector('.chart-title-input').value, chartSubtitle = section.querySelector('.chart-subtitle-input').value;
    ...
    function applyCalculatedColumn() {
        ...
        try {
            ...
            // Se a tabela modificada é a que está sendo exibida, atualize a visualização
            const activeDataName = Object.keys(alasql.tables).find(key => alasql.tables[key].data === currentData);
            if (activeDataName === tableName) {
                currentData = newData;
    ...
    function applyFilter(closeModal = true) {
        ...
        let sourceTable = initialTableName;
        if (!alasql.tables[sourceTable]) {
            const firstTable = Object.keys(alasql.tables)[0];
            if (!firstTable) { alert("Nenhuma tabela carregada para filtrar."); return; }
            sourceTable = firstTable;
        }

        if (!column) {
            currentData = alasql.tables[sourceTable].data;
        } else {
            let query = `SELECT * FROM \`${sourceTable}\` WHERE \`${column}\` `;
            switch (condition) {
    ...
    // --- Funções de Salvar e Carregar Sessão ---
    ...
    function saveState() {
        ...
        // 1. Salvar dados de todas as tabelas
        for (const tableName in alasql.tables) {
            if (alasql.tables.hasOwnProperty(tableName) && alasql.tables[tableName].data) {
                state.alasqlTables[tableName] = alasql.tables[tableName].data;
            }
        }

        // 2. Salvar configurações de todos os gráficos
        document.querySelectorAll('.chart-analysis-section').forEach(section => {
            ...
            const chartConfig = {
                ...
                title: section.querySelector('.chart-title-input').value,
            ...
            section.querySelectorAll('.series-control').forEach(seriesControl => {
                const series = {
                    xAxis: seriesControl.querySelector('select[name="x-axis"]').value,
                    yAxis: seriesControl.querySelector('select[name="y-axis"]').value,
                    aggregation: seriesControl.querySelector('select[name="aggregation"]').value,
                    seriesLabel: seriesControl.querySelector('input[name="series-label"]').value,
                    seriesType: seriesControl.querySelector('select[name="series-type"]').value,
                    secondaryAxis: seriesControl.querySelector('input[name="secondary-axis"]').checked,
                    color: seriesControl.querySelector('input[name="color"]').value
                };
        ...
        // 3. Criar a tag de script com o estado
        const stateScript = document.createElement('script');
        stateScript.id = 'savedState';
        stateScript.type = 'application/json';
        stateScript.textContent = JSON.stringify(state, null, 2);

        // 4. Injetar o script em um clone do documento para gerar o conteúdo do arquivo
        const docClone = document.cloneNode(true);
        const originalJsonScript = docClone.getElementById('jsonData');
        if(originalJsonScript) originalJsonScript.remove(); // Remove dados originais para economizar espaço
        docClone.head.appendChild(stateScript);
        
        const htmlContent = `<!DOCTYPE html>\n${docClone.documentElement.outerHTML}`;

        // 5. Disparar o download
    ...
    function loadStateAndRestoreUI() {
        const stateElement = document.getElementById('savedState');
        if (!stateElement) return;

        const state = JSON.parse(stateElement.textContent);
        if (state.version !== "6.2.0") {
            console.warn("Versão do estado salvo é diferente da versão da aplicação. Pode haver incompatibilidades.");
        }

        // 1. Restaurar Tabelas do AlaSQL
        alasql.tables = {};
        Object.keys(state.alasqlTables).forEach(tableName => {
            alasql.tables[tableName] = { data: state.alasqlTables[tableName] };
        });
        updateTableListUI();
        const firstTableName = Object.keys(state.alasqlTables)[0] || initialTableName;
        currentData = alasql.tables[firstTableName]?.data || [];
        let newColumns = currentData.length > 0 ? Object.keys(currentData[0]).map(name => ({ originalName: name, displayName: name })) : [];
        updateColumnStructure(newColumns);

        // 2. Restaurar Formatação Condicional
        conditionalFormattingRules = state.conditionalFormatting || [];
        renderCondFormatRules();

        // 3. Restaurar Editor SQL
        if (state.sqlEditorContent) {
            document.getElementById('sql-editor').value = state.sqlEditorContent;
        }

        // 4. Restaurar Gráficos
        document.getElementById('charts-container').innerHTML = '';
        chartInstances = {};
        chartAnalysisCounter = 0;
        if (state.charts && state.charts.length > 0) {
            state.charts.forEach(chartConfig => {
                addChartAnalysis(chartConfig.id);
                restoreChartUI(chartConfig);
            });
        }

        // 5. Atualizações finais da UI
        renderTable();
        updateStatus();
        if (state.charts) {
            state.charts.forEach(chartConfig => renderChart(chartConfig.id));
        }
    }
    
    function restoreChartUI(config) {
        const id = config.id;
        const section = document.getElementById(`chart-section-${id}`);
        if (!section) return;

        // Restaurar configurações gerais do gráfico
        section.querySelector('.chart-data-source').value = config.dataSource;
        section.querySelector('.chart-data-source').dispatchEvent(new Event('change'));
        document.getElementById(`type-${config.chartType}-${id}`).checked = true;
        document.getElementById(`type-${config.chartType}-${id}`).dispatchEvent(new Event('change'));
        if (config.groupBy) section.querySelector('select[name="group-by"]').value = config.groupBy;
        if (config.title) section.querySelector('.chart-title-input').value = config.title;
        if (config.subtitle) section.querySelector('.chart-subtitle-input').value = config.subtitle;
        section.querySelector('.y-axis-auto').checked = config.yAxisAuto;
        section.querySelector('.y-axis-auto').dispatchEvent(new Event('change'));
        ...
        if (config.series && config.series.length > 0) {
            config.series.forEach((seriesConfig, index) => {
                ...
                newSeriesControl.querySelector('select[name="x-axis"]').value = seriesConfig.xAxis;
                newSeriesControl.querySelector('select[name="y-axis"]').value = seriesConfig.yAxis;
                newSeriesControl.querySelector('select[name="aggregation"]').value = seriesConfig.aggregation;
                newSeriesControl.querySelector('input[name="series-label"]').value = seriesConfig.seriesLabel;
                newSeriesControl.querySelector('select[name="series-type"]').value = seriesConfig.seriesType;
                newSeriesControl.querySelector('input[name="secondary-axis"]').checked = seriesConfig.secondaryAxis;
                newSeriesControl.querySelector('input[name="color"]').value = seriesConfig.color;
            });
        }
    }
        ...
        <div class="container mx-auto">
            <div class="flex flex-wrap justify-between items-center gap-4">
                ...
                <div class="flex items-center space-x-2 flex-wrap gap-y-2">
                    ...
                    <button id="btn-save-session" class="px-4 py-2 text-sm font-medium text-white bg-cyan-600 rounded-md hover:bg-cyan-700">Salvar Sessão</button>
    ...
    <div id="cond-format-modal" class="modal hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-30">
        <div class="relative top-20 mx-auto p-5 border w-[500px] shadow-lg rounded-md bg-white">
            ...
            <div class="mt-4">
                ...
                <div class="grid grid-cols-4 gap-2 items-end p-3 border rounded-md">
...
Function Start-WebApp {
    ...
    $OpenFileDialog.Title = "Power-Graphx: Selecione um arquivo CSV ou uma sessão .html salva"

    if ($OpenFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        $FilePath = $OpenFileDialog.FileName
        
        # Se for um HTML salvo, apenas o abra.
        if ($FilePath.EndsWith(".html")) {
            Write-Host "Abrindo sessão salva..." -ForegroundColor Green
            Start-Process $FilePath
            return
        }

        # Se for um CSV, processe-o e gere o HTML.
        try {
            ...
            $JsonColumnStructure = $Data[0].PSObject.Properties | ForEach-Object { [PSCustomObject]@{ originalName = $_.Name; displayName  = $_.Name } } | ConvertTo-Json -Compress
