...
Function Get-HtmlTemplate {
    ...
    window.onload = () => {
        try {
            ...
            initializeApp();
            loadStateAndRestoreUI(); // Tenta carregar uma sessão salva por último
        } catch (e) {
    ...
    function handleDataSourceUpdate(event) {
        ...
        reader.onload = function(e) {
            const fileContent = e.target.result;
            try {
                // Tenta parsear com ponto e vírgula primeiro, depois vírgula
                ...
                originalData = JSON.parse(JSON.stringify(newData)); // Atualiza o "backup" para o botão de reset

                // Atualiza a UI
                alert("Fonte de dados atualizada com sucesso! Recalculando visualizações...");
                document.getElementById('sql-editor').value = `SELECT * FROM ${initialTableName};`;
                runQueryAndUpdateUI(); // Isso irá redesenhar a tabela e todos os gráficos
    ...
    function handleCellEdit(e) {
        ...
        if(rowIndex >= 0 && colName && currentData[rowIndex]) {
            ...
            // A atualização da tabela AlaSQL é feita sob demanda (ex: ao rodar SQL ou atualizar fonte)
            document.querySelectorAll('.chart-analysis-section').forEach(section => renderChart(section.dataset.id));
        }
    }
    ...
    function saveState() {
        ...
        // Remove o JSON de dados iniciais, pois o estado salvo é mais completo
    ...
    function restoreChartUI(id, config) {
        ...
        section.querySelector('.chart-title-input').value = config.title;
