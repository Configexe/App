...
# Versão: 21.0 - Ordenação Personalizada no Eixo do Gráfico
# Autor:Configexe
...
Function Load-CSVData {
    ...
    if ($OpenFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ($null -ne $Data -and $Data.Count -gt 0) {
            ...
            $MenuItemsToEnable | ForEach-Object { $_.Enabled = $true }
            $Global:IsDataFiltered = $false
        } else {
            ...
            $MenuItemsToEnable | ForEach-Object { $_.Enabled = $false }
        }
    }
}
...
Function Sort-Data {
    ...
    # Criar form de diálogo para ordenação
    $form = New-Object System.Windows.Forms.Form
    $form.Text = "Ordenar Dados"
...
Function Add-CalculatedColumn {
    ...
    try {
        ...
        # Forçar a atualização do DataGridView
        $DataGridView.DataSource = $null
...
<body class="text-gray-900">
    ...
    <script>
        // --- INÍCIO DO JAVASCRIPT CORRIGIDO (VERSÃO 21.0) ---
        let chartInstance;
        let seriesCounter = 0;
        ...
        function initializeApp(rawData, columnStructure) {
            ...
            window.renderChart = function() {
                ...
                if (firstXAxisSelect && firstXAxisSelect.value) {
                     // A MUDANÇA PRINCIPAL ESTÁ AQUI: removemos o .sort() para respeitar a ordem dos dados!
                ...
                seriesControls.forEach((control) => {
                    ...
                    if (seriesType === 'line') {
                        dataset.tension = parseFloat(lineTension);
                    }
                    if (seriesType === 'bar') {
                        dataset.borderRadius = parseInt(borderRadius) || 0;
                    }
            ...
            function buildChartOptions(chartType) {
                ...
                if (chartType === 'stacked' || chartType === 'stackedLine') {
...
$Form = New-Object System.Windows.Forms.Form
$Form.Text = "Power-Graphx Editor 21.0"
$Form.Width = 1200
$Form.Height = 800
$Form.StartPosition = "CenterScreen"
...
# --- Menu de Contexto para Renomear/Remover Coluna ---
